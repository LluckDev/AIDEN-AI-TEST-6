...
class display:
    def __init__(self,grid=100,screenX=100,screenY=100,bgc="#ffffff",fullscreen=True):
        ...
        self.canvis = Canvas(self.window, width=self.window.winfo_width() / 1.25, height=self.window.winfo_height() / 1.25,bg=bgc)
        if fullscreen == True:
    ...
    def update(self):
        ...
        for i in range(self.numb):
            ...
            if self.data[i][0] == 4:
                self.canvis.coords(self.storage["{0}".format(str(self.data[i][1]))],self.data[i][2] * self.winx/self.incrment, self.data[i][3]* self.winy/self.incrment, self.data[i][4] * self.winx/self.incrment,self.data[i][5] * self.winy/self.incrment)
                ...
                self.canvis.itemconfig(self.storage["{0}".format(str(self.data[i][1]))],font=("Purisa",int(self.data[i][4]*(self.winx+self.winy)/600)))
                self.canvis.coords(self.storage["{0}".format(str(self.data[i][1]))],self.data[i][2] * self.winx/self.incrment,self.data[i][3]* self.winy/self.incrment)
                ...
                self.canvis.itemconfig(self.extra["{0}".format(str(self.data[i][1]))],
                                       font=("Purisa", int((((self.data[i][2] - self.data[i][4]) + (self.data[i][3] - self.data[i][5]))/2)*  (self.winx + self.winy) / 1000)))
                self.canvis.coords(self.extra["{0}".format(str(self.data[i][1]))],
                                   (((self.data[i][4]-self.data[i][2])/2)+self.data[i][2]) * self.winx / self.incrment,
                                   (((self.data[i][5]-self.data[i][3])/2)+self.data[i][3]) * self.winy / self.incrment)
    ...
    def line(self,tag,x,y,xp,yp,color="#000000"):
        self.storage["{0}".format(str(tag))] = self.canvis.create_line(100,100,100,100)
        self.location["{0}".format(str(tag))] = self.numb
        self.data.append([4,tag,x,y,xp,yp,color,"#000000",0])
        self.numb += 1

    def rect(self,tag,x,y,xp,yp,fill="#ffffff",stroke="#000000"):
        self.storage['{0}'.format(str(tag))] = self.canvis.create_rectangle(100,100,100,100,fill=fill,outline=stroke)
        self.location['{0}'.format(str(tag))] = self.numb
        self.data.append([4,tag,x,y,xp,yp,fill,stroke,0])
        self.numb += 1

    def ellipse(self,tag,x,y,xp,yp,fill="#ffffff",stroke="#000000"):
    ...
    def text(self,tag,x,y,size,text=10,fill="#ffffff",stroke="#000000",font="Lato"):

        self.storage['{0}'.format(str(tag))] = self.canvis.create_text(x,y,text=str(text))
        self.location['{0}'.format(str(tag))] = self.numb
        self.data.append([2,tag, x, y, size,fill, stroke,text,font])
        self.numb += 1
    def triangle(self,tag,x1,y1,x2,y2,x3,y3,fill="#000000"):
        self.storage['{0}'.format(str(tag))] = self.canvis.create_polygon(x1,y1,x2,y2,x3,y3,fill=fill)
        self.location['{0}'.format(str(tag))] = self.numb
        self.data.append([6, tag, x1, y1,x2 , y2,x3,y3, fill])
        self.numb += 1
    def hitbox(self,tag,x1,y1,x2,y2,funct):
        ...
        self.numb += 1
    def button(self,tag,x1,y1,x2,y2,funct,fill="#ffffff",stroke="#000000",Text=None):
        self.storage['{0}'.format(str(tag))] = self.canvis.create_rectangle(100, 100, 100, 100, fill=fill,outline=stroke)
        self.location['{0}'.format(str(tag))] = self.numb
        self.data.append(["4b", tag, x1, y1, x2, y2, fill, stroke, funct])
        self.numb += 1
    ...
    def settype(self,tag,x=None,y=None,xp=None,yp=None,fill=None,text=None):
        ...
        if text != None:
            self.self.canvis.itemconfig(self.storage['{0}'.format(str(tag))],text=text)
